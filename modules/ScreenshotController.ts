			...
			if (getScreenCache.sessionId === sessionId && getScreenCache.tabId === id) {
				...
				if (getScreenCache.screen == null) {
					...
					try {
						...
						if (debugScreenCache) console.log('getScreen then handler added');
						callback(getScreenCache.screen, getScreenCache.pixRat);
						getScreenCache = null;
						if (debugScreenCache) console.log('Screen got from cache!!');
						return;
					} catch (e) {
						getScreenCache = null;
						if (debugScreenCache) console.error('getScreen cache promise failed:', e);
					}
				}
