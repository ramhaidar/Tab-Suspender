...
class TabCapture {
	...
	async captureTab(tab: chrome.tabs.Tab, options?): Promise<void> {
		if (options == null || options.checkActiveTabNotChanged !== true) return this._captureTab(tab, options);
			...
			if (tab_ != null) return this._captureTab(tab_, options);
				...
				chrome.tabs.query({ currentWindow: true, active: true }, async (tabsResult) => {
					...
					if (tabsResult.length > 0 && tabsResult[0] != null) {
						...
						if (actualTab.id !== tab.id) {
							console.warn(`Active tab to Capture already changed [${tab.id}] != [${actualTab.id}]`, {
								...actualTab,
						...
						if (actualTab.url !== tab.url) {
							console.warn(`Active tab URL to Capture already changed [${tab.url}] != [${actualTab.url}]`, {
								...actualTab,
								favIconUrl: undefined
							});
								...
								try {
									chrome.tabs.captureVisibleTab(
										...
										(screen: string) => {
											...
											if (debug) console.trace(`_captureTab()->captured... ${screen.length}`);
												...
												chrome.scripting
													...
													.then(async (result) => {
														try {
															...
															if (debug) console.log(`_captureTab() -> captured -> saved!`);
													...
													.catch((e) => {
														hasLastError(
															TabCapture.expectedInjectExceptions,
															e,
															`'return window.devicePixelRatio;', ActualTab: ${JSON.stringify({ ...actualTab, favIconUrl: undefined })}`
														);
														reject(e);
													});
											} else {
			...
			chrome.scripting
				...
				.catch((e) => {
					hasLastError(
						TabCapture.expectedInjectExceptions,
						e,
						`Error while injecting 'lib/h2c.js', ActualTab: ${JSON.stringify({ ...tab, favIconUrl: undefined })}`
					);
				});
		} catch (e) {
