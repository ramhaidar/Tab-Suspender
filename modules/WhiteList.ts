...
class WhiteList {
					...
					if (localExceptionPatterns != null) {
						...
						for (let i = 0; i < localExceptionPatterns.length; i++) {
							if (localExceptionPatterns[i] != null && localExceptionPatterns[i].length > 0) {
								...
								if (patternObject != null) this.patternList.push(patternObject);
							}
						}
					}
				} else this.patternList = [];
			})
			.catch(console.error);
	}
	...
	isURIException(url): boolean {
		...
		try {
			...
			if (this.findAffectdPatternIndexByUrl(url) != null) filterException = true;
		} catch (ex) {
	...
	async persist() {
		const patterns = [];
		for (const i in this.patternList) patterns.push(this.patternList[i].pattern);
