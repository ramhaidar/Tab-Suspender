...
describe('TabCapture Race Condition Tests', () => {
	...
	describe('captureTab promise completion', () => {
		it('should not resolve until all async operations complete', async () => {
			...
			expect(operations).toEqual(['captureTab-called', 'addScreen-completed', 'captureTab-resolved']);
		});
		...
		it('should handle multiple concurrent captures correctly', async () => {
			...
			const captures = [tabCapture.captureTab(mockTab), tabCapture.captureTab(mockTab), tabCapture.captureTab(mockTab)];
	...
	describe('14.4 — Tab closed during capture: no unhandled exception', () => {
		...
		it('rejects gracefully when active tab changes between query and capture', async () => {
			...
			let rejected = false;
			await tabCapture.captureTab(originalTab).catch(() => {
				rejected = true;
			});
		...
		it('void-style capture with closed tab does not crash the extension', async () => {
			...
			await tabCapture.captureTab(mockTab).catch((e: Error) => {
				caught = e;
			});
