...
describe('IndexedDBProvider Promise Handling Tests', () => {
	...
	describe('putV2', () => {
		...
		it('should handle multiple simultaneous writes correctly', async () => {
			...
			const promises = [provider.putV2([createQuery(1)]), provider.putV2([createQuery(2)]), provider.putV2([createQuery(3)])];
	...
	describe('getTransaction', () => {
		it('should reject when initializedPromise fails', async () => {
			...
			await expect(provider.getTransaction(['screens'], 'readonly')).rejects.toBeDefined();
