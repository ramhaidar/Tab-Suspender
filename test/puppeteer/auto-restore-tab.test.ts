...
async function main(): Promise<void> {
	...
	try {
		...
		const restoredUrl = await waitForAnyTabToLeaveParked(browser, extensionId, 'example.com', 20000).catch(async () => {
		...
		runner.softAssert(restored === (originalValue ?? false), `autoRestoreTab restored to ${originalValue ?? false} (got: ${restored})`);
	} finally {
		await browser.close();
		log('Browser closed');
	}
