...
async function main(): Promise<void> {
	...
	try {
		...
		const discardedTab = tabsAfterDiscard.find((t) => t.id === parkTabId && t.url && t.url.startsWith(parkPrefix));
		...
		const restoredUrl = await waitForAnyTabToLeaveParked(browser, extensionId, 'example.com', 25000).catch(async () => {
