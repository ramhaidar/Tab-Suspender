...
async function main(): Promise<void> {
	...
	try {
		...
		const restoredUrl = await waitForTabToRestore(browser, parkedTabId, 20000).catch(async () => {
		...
		const stillParked = Object.values(tabInfosAfter).some((info) => info._parked === true && !info._closed);
