 ...
 *
 * Setup:
 *   cd test/puppeteer && pnpm install
 *   pnpm run build   (in project root)
 *
 * Run:
 *   pnpm exec tsx suspend-discard.test.ts
 */
...
async function suspendDiscardUndiscard(
	...
	const yandexParkTabs = freshParkTabs.filter((t) => new URL(t.url).searchParams.get('url')?.includes('yandex'));
...
async function checkParkedTabIds(
	...
	const prefix = parkUrlPrefix(extensionId);
	const parkChromeTabIds = new Set(chromeTabs.filter((t) => t.url?.startsWith(prefix)).map((t) => t.id));

	runner.assert(parkChromeTabIds.has(parkTabBId), `${label}: Park tab B (${parkTabBId}) present in Chrome tab list`);
	runner.assert(parkChromeTabIds.has(parkTabCId), `${label}: Park tab C (${parkTabCId}) present in Chrome tab list`);
	...
	runner.softAssert(parkedInfoIds.includes(parkTabBId), `${label}: TabInfo with parked=true exists for park B (${parkTabBId})`);
	runner.softAssert(parkedInfoIds.includes(parkTabCId), `${label}: TabInfo with parked=true exists for park C (${parkTabCId})`);
}
...
async function waitAndCheckCleanup(
	...
	runner.assert(tabInfosAfter[String(parkTabBId)]?._parked === true, `${label} 1.3.1: Park tab B TabInfo retained after cleanup`);
	runner.assert(tabInfosAfter[String(parkTabCId)]?._parked === true, `${label} 1.3.1: Park tab C TabInfo retained after cleanup`);
	...
	for (const tab of nonParkTabs) {
		...
		if (info) {
			runner.softAssert(info._id === tab.id, `${label} 1.3.3: TabInfo._id (${info._id}) matches Chrome tab id (${tab.id})`);
			runner.softAssert(
				typeof info._time === 'number' && info._time >= 0,
				`${label} 1.3.3: Non-park tab ${tab.id} TabInfo._time is a valid timestamp (got: ${info._time})`
			);
			runner.softAssert(info._parked !== true, `${label} 1.3.3: Non-park tab ${tab.id} TabInfo._parked is not true (got: ${info._parked})`);
		}
	}
}

async function checkScreenshots(browser: Browser, runner: TestRunner, label: string, extensionId: string): Promise<void> {
	...
	for (const page of parkPages.slice(0, 2)) {
		...
		runner.softAssert(loaded, `${label} 1.4: Screenshot loaded on park page (${page.url().slice(0, 70)}...)`);
	}
}

// ─── Full cycle (steps 1–1.4) ─────────────────────────────────────────────────

async function runFullCycle(browser: Browser, runner: TestRunner, cycleLabel: string, extensionId: string): Promise<CycleResult> {
	...
	await waitAndCheckCleanup(browser, runner, `${cycleLabel} 1.3`, extensionId, originalTabBId, originalTabCId, parkTabBId, parkTabCId);
...
async function main(): Promise<void> {
	...
	try {
		...
		runner.assert(restoredExtensionId === extensionId, `Phase-2: Extension ID unchanged after restart (${restoredExtensionId})`);
		...
		await waitAndCheckCleanup(
			...
			'2.1',
			extensionId,
			phase1Result.parkTabBId,
			phase1Result.parkTabCId, // stale → should be cleaned
			newParkTabBId,
			newParkTabCId // current → should remain
		);
		...
		for (const tab of nonParkTabsAfterRestart) {
			...
			if (info) {
				...
				runner.softAssert(info._parked !== true, `2.4: Non-park tab ${tab.id} TabInfo._parked is not true (got: ${info._parked})`);
			}
		}
