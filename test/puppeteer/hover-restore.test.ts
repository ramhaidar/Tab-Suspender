...
async function main(): Promise<void> {
	...
	try {
		...
		runner.assert((await getSetting(browser, 'restoreOnMouseHover')) === true, 'restoreOnMouseHover set to true');
		...
		await parkPage
			.waitForFunction(() => !!(document.getElementById('resoteImg') as HTMLImageElement | null)?.onmouseover, { timeout: 8000 })
			.catch(async () => {
		...
		runner.assert((await getSetting(browser, 'restoreOnMouseHover')) === false, 'restoreOnMouseHover set to false');
