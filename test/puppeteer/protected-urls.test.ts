...
const ALLOWED_URLS = ['https://example.com/', 'http://example.com/', 'https://github.com/'];

async function isAllowedForPark(browser: Parameters<typeof evalInSW>[0], url: string): Promise<boolean> {
	const result = await evalInSW<boolean>(
		browser,
		`
...
async function main(): Promise<void> {
	...
	try {
		...
		for (const url of PROTECTED_URLS) {
			...
			runner.assert(!allowed, `isTabURLAllowedForPark returns false for: ${url}`);
		}
		...
		await settingsPage
			.goto('chrome://settings/', { waitUntil: 'domcontentloaded', timeout: 10000 })
		...
		for (const url of ALLOWED_URLS) {
			const allowed = await isAllowedForPark(browser, url);
			runner.assert(allowed, `isTabURLAllowedForPark returns true for: ${url}`);
		}
	} finally {
		await browser.close();
		log('Browser closed');
