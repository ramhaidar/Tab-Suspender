...
async function main(): Promise<void> {
	...
	try {
		...
		runner.assert((await getSetting(browser, 'reloadTabOnRestore')) === true, 'reloadTabOnRestore=true set');
		...
		const restoredUrl = await waitForAnyTabToLeaveParked(browser, extensionId, '127.0.0.1', 25000).catch(async () => {
