...
async function main(): Promise<void> {
	...
	try {
		...
		const { tabId: tabIdA, discarded: discardedA } = await openBackgroundTab(browser, testUrl);

		log(`  Tab ${tabIdA}: discarded=${discardedA}`);
		runner.softAssert(!discardedA, `Background tab NOT auto-discarded when setting is false (discarded=${discardedA})`);
		...
		runner.assert((await getSetting(browser, 'openUnfocusedTabDiscarded')) === true, 'openUnfocusedTabDiscarded set to true');

		const { tabId: tabIdB, discarded: discardedB } = await openBackgroundTab(browser, testUrl, 12000);

		log(`  Tab ${tabIdB}: discarded=${discardedB}`);
		runner.assert(discardedB === true, `Background tab auto-discarded when openUnfocusedTabDiscarded=true (discarded=${discardedB})`);
