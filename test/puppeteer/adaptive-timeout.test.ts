...
async function openAndSetup(browser: Awaited<ReturnType<typeof launchBrowser>>): Promise<{ page: Page; blank: Page; tabId: number }> {
	const page = await browser.newPage();
	await page.goto(TARGET_URL, { waitUntil: 'domcontentloaded', timeout: 20000 }).catch((e) => log(`  nav: ${(e as Error).message}`));
	await sleep(1500);
	...
	const actual = await evalInSW<string>(
		browser,
		`(function() {
    const info = tabManager.getTabInfoById(${tabId});
...
async function main(): Promise<void> {
	...
	try {
		...
		const urlAfterTick1 = await evalInSW<string>(
			browser,
			`(async () => {
      ...
      return t ? t.url : null;
		...
		const urlAfterTick2 = await evalInSW<string>(
			browser,
			`(async () => {
      const t = await chrome.tabs.get(${tabIdB}).catch(() => null);
      return t ? t.url : null;
		...
		while (Date.now() < deadlineB) {
			const url = await evalInSW<string>(
				browser,
				`(async () => {
        const t = await chrome.tabs.get(${tabIdB}).catch(() => null);
        return t ? t.url : null;
		...
		if (originalAdaptive != null) await setSetting(browser, 'adaptiveSuspendTimeout', originalAdaptive).catch(() => {});
		if (originalTimeout != null) await setSetting(browser, 'timeout', originalTimeout).catch(() => {});
		await browser.close();
		log('Browser closed');
	}
