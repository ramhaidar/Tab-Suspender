...
async function screenshotExists(browser: Awaited<ReturnType<typeof launchBrowser>>, tabId: number): Promise<number | null> {
	return evalInSW<number | null>(
		browser,
		`(async () => {
...
async function main(): Promise<void> {
	...
	try {
		...
		await page.goto(TARGET_URL, { waitUntil: 'load', timeout: 20000 }).catch((e) => log(`  nav note: ${(e as Error).message}`));
		...
		runner.assert((await getSetting(browser, 'screenshotsEnabled')) === false, 'screenshotsEnabled set to false');

		const page2 = await browser.newPage();
		await page2.goto(TARGET_URL, { waitUntil: 'load', timeout: 20000 }).catch((e) => log(`  nav note: ${(e as Error).message}`));
		await sleep(1500);
		...
		runner.assert(!hasScreenshotImg, 'park.html does NOT show a screenshot image when screenshotsEnabled=false');

		// Verify title and favicon ARE shown (basic fallback)
		const pageTitle = await parkPage.evaluate(() => document.title || document.querySelector('#title')?.textContent || '');
