...
async function getExtensionId(browser, maxWaitMs = 20000) {
	...
	while (Date.now() < deadline) {
		const swTarget = browser.targets().find((t) => t.type() === 'service_worker' && t.url().includes('chrome-extension://'));
		if (swTarget) {
		...
		await Promise.all(
			Array.from({ length: count }, () =>
				browser
					.newPage()
					.then((page) =>
						page.goto(targetUrl, { waitUntil: 'domcontentloaded', timeout: tabLoadTimeout }).catch((e) => log(`  tab ${i}: ${e.message}`))
					)
			)
		);
...
async function main() {
	...
	if (!fs.existsSync(EXTENSION_PATH)) {
		...
		console.error(`Run 'pnpm run build' in the project root first.`);
