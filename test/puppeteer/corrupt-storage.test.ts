 ...
 *
 * Run:
 *   cd test/puppeteer && pnpm exec tsx corrupt-storage.test.ts
 *
...
async function getAllRelevantSettings(browser: BrowserArg): Promise<Record<string, unknown>> {
	const json = await evalInSW<string>(
		browser,
		`(async () => {
...
async function setOffscreenLocalStorage(browser: Awaited<ReturnType<typeof launchBrowser>>, items: Record<string, string>): Promise<void> {
...
async function main(): Promise<void> {
	...
	try {
		...
		runner.assert(chromeTabs.filter((t) => t.url?.includes('yandex')).length >= 2, 'Setup: ≥2 yandex.com tabs open');
		...
		runner.assert(storedTimeout === USER_TIMEOUT_S, `Setup: timeout stored as ${USER_TIMEOUT_S}s (got ${storedTimeout})`);
	...
	try {
		...
		runner.assert(typeof activeB === 'boolean', `Phase-B: 'active' is boolean after full corruption (got: ${JSON.stringify(activeB)})`);

		log(
			`\n  Verdict: timeout=${timeoutB}s — ${
				timeoutB === DEFAULT_TIMEOUT_S
					? '✓ correct default applied'
					: timeoutB >= MIN_SAFE_TIMEOUT_S
						? '! safe but unexpected value'
	...
	try {
		...
		log(
			`\n  Verdict: timeout=${timeoutC}s — ${
				timeoutC === 30
					? '✗ BUG: V2 migration re-ran and corrupted timeout!'
					: timeoutC >= MIN_SAFE_TIMEOUT_S
						? '✓ safe (fix is in place)'
