...
async function main(): Promise<void> {
	...
	try {
		...
		runner.assert(parkedAfterRemoval, 'Tab suspended after whitelist pattern removed');
		...
		const ignoreTabStillNormal = tabsAfterIgnoreSuspend.find((t) => t.id === ignoreTabId && t.url && !t.url.startsWith(parkPrefix));

		runner.assert(ignoreTabStillNormal != null, `Ignored tab ${ignoreTabId} was NOT suspended`);
