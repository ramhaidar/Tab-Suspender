...
async function main(): Promise<void> {
	...
	try {
      ...
      if (tabs[0]) parkTabs(tabs[0]);
		...
		runner.assert(controlStillActive != null && !controlStillActive.url.startsWith(parkPrefix), 'Active (control) tab was NOT suspended');
		...
		runner.assert(parkCountAfter === 0, `All park.html tabs restored after unsuspendTabs() (remaining: ${parkCountAfter})`);
      ...
      if (tabs[0]) parkTab(tabs[0], tabs[0].id);
		...
		const currentTabParked = tabsAfterCurrentSuspend.find((t) => t.id === currentTabId && t.url && t.url.startsWith(parkPrefix));

		runner.assert(currentTabParked != null, `Tab ${currentTabId} (was active) is now parked`);
	} finally {
		await browser.close();
		log('Browser closed');
	}
