...
async function getTabFavIcon(browser: BrowserLike, tabId: number): Promise<string | null> {
	const json = await safeEval<string>(
		browser,
		`(async () => {
...
async function main(): Promise<void> {
	...
	try {
		...
		runner.softAssert(
			!r1.lost,
			`[R1] favicon intact after suspend+restore on repo root (park icon ${r1.parkIconParam ? 'present' : 'MISSING'})`
		);
		...
		await page2
			.goto('https://github.com/sergey-drpa/gpu-code-docker/blob/main/docker-compose.yaml', {
		...
		log(
			`  ${'Round'.padEnd(6)} ${'Label'.padEnd(26)} ${'Before'.padEnd(22)} ${'park icon='.padEnd(22)} ${'After restore'.padEnd(22)} Lost?`
		);
		log(`  ${'─'.repeat(110)}`);
		for (const r of results) {
			const lostMark = r.lost ? '*** LOST ***' : 'ok';
			log(
				`  ${String(r.round).padEnd(6)} ${r.label.padEnd(26)} ${favStr(r.favBeforeSuspend).padEnd(22)} ${favStr(r.parkIconParam).padEnd(22)} ${favStr(r.favAfterRestore).padEnd(22)} ${lostMark}`
			);
		}
		log('═'.repeat(80));
		...
		runner.assert(lostRounds.length === 0, `All ${results.length} rounds: no favicon loss detected (${lostRounds.length} failures)`);
