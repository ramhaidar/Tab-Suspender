...
const SVG_WITHOUT_DIMS =
	'data:image/svg+xml;base64,' +
	Buffer.from(
		'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">' + '<circle cx="16" cy="16" r="14" fill="#2ecc71"/>' + '</svg>'
	).toString('base64');

// ─── Helpers ──────────────────────────────────────────────────────────────────

async function safeEvalInSW<T = unknown>(browser: Parameters<typeof evalInSW>[0], expr: string): Promise<T | null> {
	try {
...
async function getChromeFavIcon(browser: Parameters<typeof evalInSW>[0], tabId: number): Promise<string | null> {
	const json = await safeEvalInSW<string>(
		browser,
		`(async () => {
...
async function testParkFaviconRendering(
	...
	const parkUrl =
		...
		'&icon=' +
		encodeURIComponent(iconDataUrl);
		...
		const result = await testPage.evaluate(() => {
			...
			if (href?.startsWith('data:image/png;base64,')) {
				...
				try {
					...
					if (binary.length >= 24) {
						const w =
							((binary.charCodeAt(16) << 24) | (binary.charCodeAt(17) << 16) | (binary.charCodeAt(18) << 8) | binary.charCodeAt(19)) >>> 0;
						const h =
							((binary.charCodeAt(20) << 24) | (binary.charCodeAt(21) << 16) | (binary.charCodeAt(22) << 8) | binary.charCodeAt(23)) >>> 0;
...
async function main(): Promise<void> {
	...
	try {
		...
		runner.assert(bPassed, `[Phase B] SVG without dimensions → non-empty PNG favicon (got ${JSON.stringify(resB.canvasSize)})`);
		...
		const initialTabJson = await safeEvalInSW<string>(
			browser,
			`(async () => {
      ...
      return JSON.stringify(t ? { id: t.id, favIconUrl: t.favIconUrl ?? null } : null);
		...
		const tabUrlAfterRestore = await safeEvalInSW<string>(
			browser,
			`(async () => {
      const t = await chrome.tabs.get(${tabId}).catch(() => null);
      return t ? t.url : null;
