...
async function main(): Promise<void> {
	...
	try {
		...
		await pageA.goto(TARGET_URL, { waitUntil: 'domcontentloaded', timeout: 20000 }).catch((e) => log(`  nav: ${(e as Error).message}`));
		await sleep(1500);
		...
		await pageB.goto(TARGET_URL, { waitUntil: 'domcontentloaded', timeout: 20000 }).catch((e) => log(`  nav: ${(e as Error).message}`));
		await sleep(1500);
		...
		if (originalPinned != null) await setSetting(browser, 'pinned', originalPinned).catch(() => {});
