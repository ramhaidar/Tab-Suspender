...
async function main(): Promise<void> {
	...
	try {
		...
		const parkDataA = await parkPageA.evaluate(
			() =>
		...
		runner.assert(parsedA.startDiscarded === false, `parkData.startDiscarded=false when setting disabled (got: ${parsedA.startDiscarded})`);
		...
		const parkDataB = await parkPageB.evaluate(
			() =>
				new Promise<string>((resolve) => {
					const params = new URLSearchParams(location.search);
					chrome.runtime.sendMessage(
		...
		runner.assert(parsedB.startDiscarded === true, `parkData.startDiscarded=true when setting enabled (got: ${parsedB.startDiscarded})`);
		runner.assert(parsedB.startAge != null && parsedB.startAge < 15000, `startAt is within 15s window (startAge=${parsedB.startAge}ms)`);
		...
		while (Date.now() < deadline) {
			const info = await evalInSW<string>(
				browser,
				`(async () => {
		...
		runner.assert(finalDiscarded, `[AutomaticTabCleaner:DiscardTab] successfully discards the park.html tab`);
