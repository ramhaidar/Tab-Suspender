...
(function () {
	...
	force('String', String, [
		...
		'reduce',
		'reduceRight'
	])('Number', Number, ['toExponential', 'toFixed', 'toLocaleString', 'toPrecision'])('Function', Function, ['apply', 'call', 'bind'])(
		'RegExp',
		RegExp,
		['exec', 'test']
	)('Object', Object, [
...
Array.implement({
	...
	flatten: function () {
		...
		for (var i = 0, l = this.length; i < l; i++) {
			...
			array = array.concat(
				type == 'array' || type == 'collection' || type == 'arguments' || instanceOf(this[i], Array) ? Array.flatten(this[i]) : this[i]
			);
		}
		return array;
	},
...
(function () {
	...
	local.setDocument = function (document) {
		...
		if (features.isHTMLDocument) {
			...
			features.nativeMatchesSelector =
				root.matchesSelector || /*root.msMatchesSelector ||*/ root.mozMatchesSelector || root.webkitMatchesSelector;
	...
	local.matchNode = function (node, selector) {
		...
		for (i = 0; (currentExpression = expressions[i]); i++) {
			if (currentExpression.length == 1) {
				...
				if (
					this.matchSelector(node, this.isXMLDocument ? exp.tag : exp.tag.toUpperCase(), exp.id, exp.classes, exp.attributes, exp.pseudos)
				)
					return true;
				simpleExpCounter++;
			}
