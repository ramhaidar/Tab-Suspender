...
(function () {
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		if (isDarkModeEnabled) document.getElementsByTagName('body')[0].classList.add('dark');
	});
	...
	Bundle.Button = new Class({
		...
		addEvents: function () {
			if (this.params.onclick != null) this.element.addEvent('click', this.params.onclick.bind(this));
			else
				this.element.addEvent(
					'click',
					function () {
	...
	Bundle.ColorPicker = new Class({
		...
		addEvents: function () {
			var change = function (event) {
				...
				if (this.params.remoteCall) chrome.runtime.sendMessage({ method: this.params.remoteCall });
			}.bind(this);
